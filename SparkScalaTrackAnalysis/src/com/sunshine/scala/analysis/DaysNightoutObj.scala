...
/**
 * data: hdfs:10.2.20.11:8020/oy/data/hn_passrec_daysnightout.txt
 * 前提:按天创建创建过车轨迹文件
 ...
 */
private[analysis] object DaysNightoutObject {
  
  case class DaysNightScore(hphm: String, hpzl: String, day: String, hour: String, score: Int)
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(args.length <= 0){
      println("DaysNightout <File> <Master> <DriverMemory> <ExecutorMemory>")
      System.exit(0)
    }
    ...
    // Test view data
    val df = spark.read.csv(_file); //DateFrame
    df.show(10)
    ...
    // print
    dayResComRdd.take(200).foreach(println)
    
    dayResComRdd.saveAsTextFile("file:/opt/1.txt");
